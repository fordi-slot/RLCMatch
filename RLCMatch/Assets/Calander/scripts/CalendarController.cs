...
public class CalendarController : MenuScreen
{
    ...
    private CalendarArgs.CalendarTypye m_type;
        ...
        for (int i = 0; i < _totalDateNum; i++)
        {
            ...
            if (i >= index)
            {
                ...
                if (thatDay.Month == firstDay.Month)
                {
                    ...
                    if (m_timeForm != null)
                        referenceTime = m_timeForm.SelectedTime;
                    else
                        referenceTime = m_referenceTime.ToShortTimeString();
                   
                    dateTimeString = _dateTime.Year + "-" + month + "-" + dateString + " " + referenceTime;
                    ...
                    itemInfo.IsValid = m_type == CalendarArgs.CalendarTypye.POSTDATE ? (dateTime >= m_referenceTime) : (dateTime <= m_referenceTime);
                    ...
                    itemInfo.Validate.AddListener((args) =>
                    {
                        ...
                        args.IsValid = m_type == CalendarArgs.CalendarTypye.POSTDATE ? (dateTime >= m_referenceTime) : (dateTime <= m_referenceTime);
    ...
    public void OpenCalendar(IUserInterface userInterface, CalendarArgs args)
    {
        ...
        m_referenceTime = args.ReferenceTime == null ? DateTime.Now : (DateTime)args.ReferenceTime;
        m_onClick = args.OnClick;
        m_type = args.CalendarType;
